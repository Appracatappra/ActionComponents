...
namespace ActionComponents
{
	...
	public class ACTileGroup : UIView, IEnumerator, IEnumerable
	{
		...
		public ACTile AddTile(ACTileStyle style, ACTileSize tileSize, string title, string subtitle, string description, Android.Content.Res.Resources resources, int resourceID)
		{
			ACTile tile;

			// Create new tile
			tile = new ACTile(this, defaultTileAppearance.Clone(), style, tileSize, title, subtitle, new UIImage(resources, resourceID), description);
		...
		/// <param name="resources">Resources.</param>
		/// <param name="resourceID">Resource ID.</param>
		public ACTile AddCustomSizedTile(int rows, int cols, ACTileStyle style, string title, string subtitle, string description, Android.Content.Res.Resources resources, int resourceID)
		{
			ACTile tile;

			// Create new tile
			tile = new ACTile(this, defaultTileAppearance.Clone(), style, ACTileSize.Custom, title, subtitle, new UIImage(resources, resourceID), description);
			tile.customRowHeight = rows;
			tile.customColumnHeight = cols;
		...
		private nfloat ReflowTilesTopToBottomAndAutoFit(nfloat width, nfloat height) {
			...
			if (title != "")
			{
				...
				titleHeight = UIFont.StringSize(title, UIFont.BoldSystemFontOfSize(appearance.titleSize), new CGSize(100f, 50f), UILineBreakMode.Clip).Height;
			...
			if (footer != "")
			{
				...
				footerHeight = UIFont.StringSize(footer, UIFont.SystemFontOfSize(appearance.footerSize), new CGSize(100f, 35f), UILineBreakMode.Clip).Height;
			}
		...
		private nfloat CalculateGroupHeight()
		{
			...
			{
				//Calculate title height
				titleHeight = UIFont.StringSize(title, UIFont.BoldSystemFontOfSize(appearance.titleSize), new CGSize(Frame.Width, _controller.Frame.Height), UILineBreakMode.Clip).Height;
			}
			...
			{
				//Set the footer to a fixed size that is 2x the line height
				footerHeight = UIFont.StringSize(footer, UIFont.SystemFontOfSize(appearance.footerSize), new CGSize(Frame.Width, _controller.Frame.Height), UILineBreakMode.Clip).Height;
			}
		...
		public override void Draw(CGRect rect)
		{
			...
			if (title != "")
			{
				...
				var titleFontAttributes = new UIStringAttributes { Font = UIFont.SystemFontOfSize(appearance.titleSize), ForegroundColor = appearance.titleColor, ParagraphStyle = titleStyle };
				...
				context.SaveState();
				var titleRect = new CGRect(0, 0, rect.Width, titleTextHeight);
			...
			if (footer != "")
			{
				...
				var footerFontAttributes = new UIStringAttributes { Font = UIFont.SystemFontOfSize(appearance.footerSize), ForegroundColor = appearance.footerColor, ParagraphStyle = footerStyle };
				...
				context.SaveState();
				var footerRect = new CGRect(0, rect.Height - footerTextHeight, rect.Width, footerTextHeight);
				context.ClipToRect(footerRect);
				new NSString(textContent).DrawString(footerRect, footerFontAttributes);
				context.RestoreState();
			...
			if (appearance.hasBackground)
			{
				...
				var groupRect = new CGRect(0, titleHeight, rect.Width, rect.Height - (titleHeight + footerHeight));
